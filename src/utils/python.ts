...
// import { XShells } from "./shells";

const python_win64_path = path.join(
  getBinariesDirectoryPath(),
  "python-win64/python.exe"
);
const python_win32_path = path.join(
  getBinariesDirectoryPath(),
  "python-win32/python.exe"
);
const python_macos_path = path.join(
  getBinariesDirectoryPath(),
  "python-mac/bin/python3"
);
// 内置依赖库路径
const python_buildin_libs = path.join(
  getBinariesDirectoryPath(),
  "python-buildin-libs"
);
// 下载路径
...
class Python {
  ...
  static getPySitePackagesDir = () => {
    ...
    if (isWin64()) {
      pythonPthPath =
        getBinariesDirectoryPath() + "/python-win64/Lib/site-packages";
    } else if (isWins()) {
      pythonPthPath =
        getBinariesDirectoryPath() + "/python-win32/Lib/site-packages";
    } else if (isMac()) {
  ...
  static installPython = () => {
    ...
    exec(command, (error: number, stdout: any, stderr: any) => {
      console.log(stdout);
      if (error) {
  ...
  static installLib = (
    libname: string,
    fcn: (error: number | null, action: string, libname: string) => void
  ) => {
  ...
  static searchLibBy = (
    libname: string,
    fcn?: (error: number | null, libname: string) => void
  ) => {
  ...
  //
  static checkInstalledLibs = (
    fcn: (error: number | null, result: string) => void
  ) => {
  ...
  static execPython = (
    ...
    onFail: Function | undefined = undefined,
  ...
  static checkSyntax = (
    code: string,
    fn: (result: string, lineNum: number, msg: string) => void
  ) => {
    ...
    exec(
      ...
      (error: number, stdout: string, stderr: string) => {
        console.log(
          `[*] checkCode, error: ${error}, stdout: ${stdout}, stderr: ${stderr}`
        );
        if (stdout) {
